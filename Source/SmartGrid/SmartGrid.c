...
void mazeEngine(int **tab, int W, int H);									 // Maze
void updateMetaData(int **metaData, int id1, int id2, int W, int H); // Maze
void setMetaData(int **metaData, int **mazeData, int W, int H);		 // Maze
void printMazeData(int **mazeData, int **CellData, int W, int H, int size);     // Maze
...
void gestionEvenement(EvenementGfx evenement)
{
	...
	case Initialisation:;
		...
		// Maze
		iniCellData(&mazeData, Wmaze, Hmaze);
		mazeEngine(mazeData, Wmaze, Hmaze);
		printMazeData(mazeData, CellData, Wmaze, Hmaze, SizeMaze);
		DeltaX = DataSizeX / 4;
		DeltaY = DataSizeY / 4;
	...
	case BoutonSouris:; //Aproximation de la position de la souris sur la grille
		...
		if (NeedScrollUpdate)
		{
			...
			if (DeltaX < 0)
				DeltaX = 0;
			if (DeltaX + WcellCap >= DataSizeX)
				DeltaX = DataSizeX - WcellCap;
			if (DeltaY < 0)
				DeltaY = 0;
			if (DeltaY + HcellCap >= DataSizeY)
				DeltaY = DataSizeY - HcellCap;
		}
	...
	while (nb_zone != 0)
	{
		...
		if (case_x > 0 && case_y > 0 && case_x < W - 1 && case_y < H - 1 && metaData[case_y][case_x] == -1)
		{
			...
			if (id1 != id2 && id1 != -1 && id2 != -1)
			{
				...
				nb_zone = nb_zone - 1;
				// printf(" %d %d verified - %d restants\n", case_x, case_y, nb_zone);
