		...
		for(int i = 0; i < target_blob->nAnts; i++){
			if(target_blob->ants[i].ttl > 0){
				...
				if(target_blob->ants[i].ttl <= 0){;}
					...
					target_blob->ants[i].moov.x += (float)((10.00 - (rand()%20))/15.0);
					target_blob->ants[i].moov.y += (float)((10.00 - (rand()%20))/15.0);
		...
		for(int tx = -radius; tx <= radius; tx++){
			if((target.x+tx) > 0 && (target.x+tx) < CW && (target.y+ty) > 0 && (target.y+ty) < CH && !(target.x == tx && target.y == ty)){
				if(cellGrid[target.y+ty][target.x+tx].type == type){
					...
					if(cellGrid[target.y+ty][target.x+tx].type == cell_food) printf("FOOD : %d %d", target.x+tx, target.y+ty);
				}
			}
		}
	}
...
coord2D getPeriphPoint(cell **cellGrid, int CW, int CH, int id, coord2D center, float targetAngle, float teta){
	coord2D result;
	result.x = 0;
	result.y = 0;
...
blob_blob newBlob(cell **cellGrid, int CW, int CH, coord2D SP, simulation sim){
	...
	cellGrid[SP.y][SP.x] = newBlobCell(cellGrid, cellGrid[SP.y][SP.x].food_amount, blob_kernel, newBLOB.id, -1, 200, cellGrid[SP.y][SP.x].mucus_amount, SP, -20);
		...
		for(int x = 0; x<CW; x++){
			for(int y = 0; y<CH; y++){
				if((cellGrid[y][x].type == cell_empty || cellGrid[y][x].type == cell_food) && n < (newBLOB.totalCells - 1) && (rand()%19) == 0){
					...
					//printf("%d, %d\n", BN, n);
					if(BN > 0){
						cellGrid[y][x] = newBlobCell(cellGrid, cellGrid[y][x].food_amount, blob_kernel, newBLOB.id, -1, 150, cellGrid[y][x].mucus_amount, new2Dcoord(x,y), -15);
						n++;
					}
				}
			}
